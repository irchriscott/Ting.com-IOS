//
//  EditMenuReviewViewController.swift
//  ting
//
//  Created by Christian Scott on 11/22/19.
...
class EditMenuReviewViewController: UIViewController, UITextViewDelegate {
    ...
    var menu: RestaurantMenu? {
        didSet {
            if let menu = self.menu {
                guard let url = URL(string: URLs.checkMenuReview) else { return }
                    ...
                    if let httpResponse = response as? HTTPURLResponse {
                        if let data = data {
                            if httpResponse.statusCode == 200 {
                                do {
                                    let menuReview = try JSONDecoder().decode(MenuReview.self, from: data)
                                    DispatchQueue.main.async { self.review = menuReview }
                                } catch {}
                            } else {
                                do {
    ...
    override func viewDidLoad() {
        ...
        self.view.addConstraintsWithFormat(format: "V:|-86-[v0]-4-[v1]-20-[v2]-20-[v3(130)]-12-[v4(55)]", views: reviewText, reviewDescriptionText, reviewRating, reviewComment, submitButton)
    }

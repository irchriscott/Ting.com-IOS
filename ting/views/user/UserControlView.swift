...
@IBDesignable open class UserControlView: UIControl {

    private var selectionIndicatorView: UIView!
    private var buttons: [UIButton]?
    private var items: [String] = ["MOMENTS", "RESTAURANTS","PROFILE"]
    
    
    @IBInspectable open var font: UIFont = UIFont(name: "Poppins-Medium", size: 16)! {
    ...
    @IBInspectable open var unselectedTitleColor: UIColor? = Colors.colorGray {
        didSet { self.configureView() }
    }
    ...
    @IBInspectable open var indicatorThickness: CGFloat = 3 {
        didSet { self.indicatorHeightConstraint.constant = self.indicatorThickness }
    }
    ...
    private func initButtons() {
        ...
        for index in 0..<self.numberOfSegments {
            ...
            button.setTitle(self.titleForSegmentAtIndex(index), for: UIControl.State.normal)
            button.addTarget(self, action: #selector(UserControlView.didTapButton(_:)), for: .touchUpInside)
    ...
    open func setSelectedSegmentIndex(_ index: Int, animated: Bool = true) {
        ...
        } else { self.selectedSegmentIndex = index }
    }
    ...
    private func configureButtons() {
        ...
        for button in buttons { self.configureButton(button) }
    }
    
    private func configureButton(_ button: UIButton) {
        button.titleLabel?.font = self.font
        button.setTitleColor(self.colorToUse(self.titleColor), for: .selected)
        button.setTitleColor(self.unselectedTitleColor, for: UIControl.State.normal)
